<template lang="pug">
v-form.add-contact.d-flex.flex-column.justify-start(
...
export default {
  ...
  watch: {
    GET_SELECTED_CONTACT(v) {
      this.getDataUserStore(v);
    }
  },

  mounted() {
    this.getDataUserStore(this.GET_SELECTED_CONTACT);
  },

  methods: {
    ...mapActions(['setSelectedContactAction', 'setSelectedContact']),

    getDataUserStore(data) {
      this.dataUser = {
        id: data.dataUser.id,
        name: data.dataUser.name,
        role: data.dataUser.role,
        bio: data.dataUser.bio,
        phones: this.setPhones(data.dataUser.phones),
        emails: this.setEmails(data.dataUser.emails)
      };
    },
    ...
    cancelChangeContact() {
      this.setSelectedContactAction('view');
    },
    ...
    async saveAllDataContacts() {
      ...
      try {
        sendDataContact = await this.$axios.put(
          `contacts/${this.dataUser.id}`,
          dataContacts
        );
      } catch (e) {
